		...
		if(count($categorias)== 0){
	 	...
	 	foreach ($categorias as $key => $value) {
			...
			$datosJson.= '[
						...
						"'.$value["descripcion"].'",
						"'.$caracteristicas.'",
						"$MXN '.number_format($value["continental_alta"]).'",
						"$MXN '.number_format($value["continental_baja"]).'",
						"$MXN '.number_format($value["americano_alta"]).'",
						"$MXN '.number_format($value["americano_baja"]).'",
						"'.$acciones.'"
